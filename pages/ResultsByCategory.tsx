...
const ResultsByCategory: React.FC = () => {
  ...
  useEffect(() => {
    if (query.category) {
      fetchResults(query.category as string, currentPage);
    }
  }, [query.category, currentPage]);

  const fetchResults = async (category: string, page: number) => {
    try {
      const response = await axios.get('/api/searchByCategory', { params: { category, page } });
